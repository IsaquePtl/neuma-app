    ...
    <div className="min-w-0 w-full max-w-full space-y-3">
      ...
      {canBookSessions ? (
        <CalBookButton
          ...
          className={
            hasBooking
              ? "pt-1 [&_button]:h-11 [&_button]:text-[0.9375rem]"
              : undefined
          }
          ...
          onBookingCancelled={async (data) => {
            ...
            try {
              if (uid && !uid.startsWith("local-")) {
                ...
                previous?.cal_booking_uid &&
                !previous.cal_booking_uid.startsWith("local-")
              ) {
        ...
        <div className="grid gap-1.5">
          <Button size="lg" disabled className="h-14 w-full gap-2 text-base font-semibold">
            {hasBooking ? "Alterar agendamento" : "Agendar sessão"}
          </Button>
