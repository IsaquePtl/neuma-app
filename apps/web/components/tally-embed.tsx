...
export type TallyFormSubmittedPayload = {
  id?: string;
  respondentId?: string;
  formId?: string;
  formName?: string;
...
export function TallyEmbed({
  ...
  params?: Record<string, string | null | undefined>;
  onReady?: () => void;
  /** Disparado no postMessage `Tally.FormSubmitted` do iframe (com answers). */
  onSubmit?: (payload: TallyFormSubmittedPayload) => void;
}) {
  ...
  useEffect(() => {
    ...
    function onMessage(event: MessageEvent) {
      ...
      try {
        ...
        if (message.event === "Tally.FormLoaded") markReady();
        if (message.event === "Tally.FormSubmitted") {
          markSubmitted(message.payload ?? {});
        }
      } catch {
        /* ignore */
      }
