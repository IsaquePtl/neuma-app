...
export function StudentTodoList({ items }: { items: StudentTodoItem[] }) {
  ...
  return (
    <Card
      className={cn(
        "flex shrink-0 flex-col gap-3 overflow-visible rounded-3xl border border-white/10 bg-white/[0.015] px-5 py-4",
        "desktop:h-auto",
      )}
    >
...
function TodoPlaceholder() {
  return (
    <li aria-hidden className="pointer-events-none">
      <div className="min-h-[4.25rem] rounded-xl border border-dashed border-white/5 bg-transparent" />
    </li>
  );
}
