...
import {
  ...
  enhanceMentorAgendaBriefing,
  generateLocalMentorAgendaBriefing,
  ...
  function load(full = false) {
    startTransition(async () => {
      try {
        ...
        setEnhancing(true);
        try {
          const upgraded = await enhanceMentorAgendaBriefing(local);
          setData(upgraded);
        } catch {
          // mantém local
        } finally {
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        >
          <RefreshCw
            className={cn(
              "size-3.5",
              (pending || enhancing) && "animate-spin",
            )}
          />
          {pending || enhancing ? "A pensar…" : "Actualizar"}
        </Button>
      </div>
      ...
      {data ? (
        <div className="space-y-3">
          ...
          {data.items.filter((_, i) => !dismissed.has(i)).length === 0 ? (
            <Card className="p-6 text-center text-sm text-muted-foreground">
